...
LEVERAGE = None  # 杠杆倍数（启动时手动输入；现金账户受 broker 最大量上限约束）
...
def prompt_capital_settings():
    """启动时交互输入账户起始资金、当前金额与杠杆；Darwinex 无利润目标，止盈/日内止损均禁用"""
    ...
    # Darwinex 无利润目标，账户止盈固定禁用
    MAX_PROFIT_AMOUNT = -1
    print("账户止盈: 已禁用（Darwinex 无利润目标，track record 越长越好）")
    
    if DAILY_LOSS_PCT > 0:
        # 日内止损金额 = min(起始资金, 当前金额) × 比例（取较小者更保守）
        MAX_DAILY_LOSS_AMOUNT = min(start_balance, current_balance) * DAILY_LOSS_PCT
        print(f"日内止损金额: ${MAX_DAILY_LOSS_AMOUNT:.2f} (= min(起始资金, 当前金额) × {DAILY_LOSS_PCT*100:.1f}%，自律风控)")
    else:
...
DB_PATH = os.path.join(get_common_files_dir(), "trading_signals_darwinex.db")
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("长桥API交易策略启动 - 模拟模式 (Darwinex)")
    print("=" * 60)
