...
//--- 输入参数
input string   DBPath = "trading_signals_goat.db";        // SQLite数据库文件名（Goat Funded Trader）
input bool     UseCommonPath = true;                   // 使用通用目录（推荐）
input int      MagicNumber = 20250501;                 // 魔术数字（与其他 EA 区分）
...
void OnTimer()
{
    ...
    // 断线期间 OnTick 不触发，定时器兜底执行日内亏损检查（重连后第一时间生效）
    if(connected)
        DailyRiskCheck();
