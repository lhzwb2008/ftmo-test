...
# 资金和风控设置（与 simulate_ftmo 同策略参数；账户规模 100K，止盈/日内止损按 FTMO 200K 同比例）
INITIAL_CAPITAL = 100000  # 初始资金（用于计算全仓盈亏）
LEVERAGE = 2 # 杠杆倍数

# 止盈止损设置（金额）
MAX_PROFIT_AMOUNT = 8000  # 止盈目标金额（设置为负数如-1则禁用止盈）
...
if platform.system() == "Windows":
    ...
    DB_PATH = os.path.join(mt5_common_path, "trading_signals_ttp.db")
else:
    # 非Windows系统：使用当前目录
    DB_PATH = "trading_signals_ttp.db"
