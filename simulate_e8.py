...
DAILY_PROFIT_CAP_PCT = 0.018  # 单日利润停止开仓比例（官方上限 2%，达到 1.8% 即停止开仓并平仓，防止利润被上限截断浪费）
...
MAX_DAILY_PROFIT_AMOUNT = -1  # 单日利润停止开仓金额（自动计算；E8 Pro 特有，负数表示禁用）
...
DAILY_PROFIT_CAP_TRIGGERED = False  # 当日是否触发了单日利润上限（E8 Pro：达到后当日平仓且不再开仓）
...
def prompt_capital_settings():
    ...
    global PROFIT_TARGET_PCT, LEVERAGE, MAX_DAILY_PROFIT_AMOUNT
    
    while True:
        try:
            phase = input("请输入当前轮次（1=挑战阶段(E8 Pro 一阶段) / funded=已通过考试）: ").strip().lower()
    ...
    print(f"单日利润停止金额: ${MAX_DAILY_PROFIT_AMOUNT:.2f} (= 起始资金 × {DAILY_PROFIT_CAP_PCT*100:.1f}%，官方单日上限 2%，达到即平仓停止开仓)")
...
DB_PATH = os.path.join(get_common_files_dir(), "trading_signals_e8.db")
    ...
    while DAILY_LOSS_MONITOR_ACTIVE:
        try:
            ...
            if is_trading_hours:
                ...
                # 检查是否触发单日利润上限（E8 Pro: 官方单日利润最多计入 2%，达到 1.8% 即平仓停止，避免利润被截断浪费）
                if MAX_DAILY_PROFIT_AMOUNT > 0 and current_daily_pnl >= MAX_DAILY_PROFIT_AMOUNT:
                    print(f"\n[{now.strftime('%Y-%m-%d %H:%M:%S')}] !!!!! [监控线程] 检测到达到单日利润上限 !!!!!")
                    print(f"[{now.strftime('%Y-%m-%d %H:%M:%S')}] [监控线程] 当日盈利: ${current_daily_pnl:.2f}")
                    print(f"[{now.strftime('%Y-%m-%d %H:%M:%S')}] [监控线程] 单日利润停止金额: ${MAX_DAILY_PROFIT_AMOUNT:.2f} (官方上限 2%，超出部分不计入目标)")
...
def run_trading_strategy(symbol=SYMBOL, check_interval_minutes=CHECK_INTERVAL_MINUTES,
    ...
    while True:
        ...
        if last_date is not None and current_date != last_date:
            ...
            DAILY_PROFIT_CAP_TRIGGERED = False  # 重置单日利润上限标志（E8 Pro）
        ...
        monitor_needed = is_after_930 and current_hour < 16 and not (PROFIT_TARGET_TRIGGERED or DAILY_STOP_TRIGGERED or DAILY_PROFIT_CAP_TRIGGERED)
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("长桥API交易策略启动 - 模拟模式 (The Trading Pit)")
    print("=" * 60)
