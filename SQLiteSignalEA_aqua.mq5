//+------------------------------------------------------------------+
//|                                      SQLiteSignalEA_aqua.mq5      |
//|                        AquaFunded 2-Step Standard 信号执行EA      |
//| Challenge: HardSLRiskPercent=0；Funded: 设 1.8~2.0（Wave Stop）   |
//+------------------------------------------------------------------+
...
input double   InitialBalance = 100000.0;              // 账户初始资金（Aqua 风控按初始资金计算）
input double   HardSLRiskPercent = 0;                  // 单笔硬止损(%)（Challenge=0；Funded Wave Stop≈2%，建议 1.8 留缓冲）

//--- 日内亏损风控参数（EA端基于真实权益逐tick监控，Python端不再负责日内止损；限额基于 InitialBalance）
input double   DailyLossPercent = 5.0;                 // 官方日内最大亏损比例(%)（Aqua Standard 官方 5；0=禁用）
...
int OnInit()
{
    ...
    Print("✅ EA初始化成功");
    Print("🧩 编译版本: 1.04（开仓前校验止损；服务器 Invalid stops 只拒绝一次，不再重试）");
    ...
    if(HardSLRiskPercent > 0)
        Print("🛡️ 单笔硬止损(Wave Stop兜底): 初始资金 $", DoubleToString(InitialBalance, 2), " × ", HardSLRiskPercent,
              "% = $", DoubleToString(InitialBalance * HardSLRiskPercent / 100.0, 2),
              " （与日亏线SL取更紧者；Challenge 请将 HardSLRiskPercent 设为 0）");
    else
        Print("🛡️ 单笔硬止损: 已禁用（Challenge 模式；Funded 请设 1.8~2.0 对应 Wave Stop）");
...
//+------------------------------------------------------------------+
//| 计算硬止损距离（价格单位）                                         |
//| Aqua Funded：Wave Stop≈合计浮亏达初始资金2%时强平；单仓时等价单笔2%  |
//| Challenge：HardSLRiskPercent=0 禁用；Funded 建议设 1.8~2.0 挂服务器 |
...
{
    sl_out = 0;
    bool daily_on = DailyLossPercent > 0;
...
void NoteOrderRejected(long signal_id, double price, double sl)
{
    ...
    if(DailyLossPercent > 0 && AccountInfoDouble(ACCOUNT_BALANCE) <= DailyTriggerFloor())
        HaltDailyNewEntries();
    MarkSignalConsumed(signal_id);
}
