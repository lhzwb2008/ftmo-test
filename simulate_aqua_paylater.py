...
ACCOUNT_PHASE = None  # "challenge" | "funded"（启动时选择；与 EA AccountPhase 必须一致）
...
# 交易时间（Pay Later 允许本人跟单，固定同一窗口即可；与回测一致）
TRADING_SESSION_HOURS = 6
...
# 与普通 Aqua Standard（simulate_aqua / Magic 20260808）完全隔离
INSTANCE_ID = None  # 1/2/3/4…
AQUA_PAYLATER_MAGIC_BASE = 20260818  # EA Magic = 此值 + INSTANCE_ID（与 mq5 默认一致）

# 日志 / 信号库路径（prompt_instance_id → apply_instance_paths 后赋值）
LOG_FILE = "trading_aqua_paylater.log"
DB_PATH = None
...
PROFIT_TARGET_TRIGGERED = False  # 是否触发了账户止盈（Challenge +3%）
DAILY_PROFIT_CAP_TRIGGERED = False  # Funded：是否触发单日利润上限（一致性）
...
def print_phase_ea_checklist(phase):
    """打印与当前阶段对应的 MT5 EA 必设参数（Challenge / Funded 必须分开）"""
    print("\n" + "-" * 60)
    print(f"【EA 参数核对】阶段 = {phase.upper()}  →  SQLiteSignalEA_aqua_paylater.mq5")
    print(f"  AccountPhase = {phase}")
    print(f"  InstanceId   = {INSTANCE_ID}")
    print(f"  InitialBalance = {ACCOUNT_START_BALANCE:.0f}（与账户起始资金一致）")
    print(f"  Leverage     = {LEVERAGE}（须与本脚本一致）")
    if phase == "challenge":
        print("  HardSLRiskPercent ≈ 0.95（浮亏封号 -1% 留缓冲）")
        print("  DailyLossPercent  = 0（Challenge 无日亏线）")
        print("  DailyProfitCapUSD = 0（Challenge 不设日盈上限）")
        print("  策略建议: 杠杆 2x，冲 +3% 目标，无日盈 cap")
    else:
        print("  HardSLRiskPercent ≈ 0.95（浮亏封号 -1% 留缓冲）")
...
def prompt_capital_settings():
    """启动时选择 Challenge/Funded、起始/当前资金、杠杆；按阶段设置止盈与日盈 cap"""
    global ACCOUNT_START_BALANCE, INITIAL_CAPITAL, MAX_PROFIT_AMOUNT
    global PROFIT_TARGET_PCT, LEVERAGE, ACCOUNT_PHASE, MAX_DAILY_PROFIT_AMOUNT
    global TRADING_START_TIME, TRADING_END_TIME

    print("\n--- Aqua Pay Later 阶段（与普通 2-Step Standard 完全分开）---")
    print("  challenge = One-Step Pay Later 考核（目标 +3%，无日亏）")
    print("  funded    = 已激活 Funded（日亏 3%，一致性 15%，建议日盈 cap）")

    while True:
        try:
            phase = input("请输入当前阶段（challenge / funded）: ").strip().lower()
            if phase in ("c", "1", "chal"):
                phase = "challenge"
            if phase in ("f", "2"):
                phase = "funded"
    ...
    # 固定交易窗口（跟单允许，与 6y 回测一致）
    TRADING_START_TIME = (9, 40)
    TRADING_END_TIME = trading_end_from_start(TRADING_START_TIME)
    print(f"交易时间(固定): {TRADING_START_TIME[0]:02d}:{TRADING_START_TIME[1]:02d} - "
          f"{TRADING_END_TIME[0]:02d}:{TRADING_END_TIME[1]:02d}")

    while True:
        try:
            default_lev = "2"
            lev_str = input(f"请输入杠杆倍数（建议 2，回车={default_lev}）: ").strip() or default_lev
            lev = float(lev_str)
            if lev > 0:
                LEVERAGE = lev
                break
            ...
            print("错误: 请输入有效数字（如 2）")
        except EOFError:
            print("错误: 无法读取输入（非交互环境），程序退出")
            sys.exit(1)

    phase_label = {"challenge": "Challenge（Pay Later 考核）", "funded": "Funded（已激活）"}[phase]
    print(f"当前阶段: {phase_label}")
    print(f"杠杆倍数: {LEVERAGE}x (手动指定)")
    print(f"⚠️ 提醒: 杠杆不会自动同步到 MT5。请在 EA 输入参数中将 Leverage 手动改为 {LEVERAGE}")
    ...
    if PROFIT_TARGET_PCT > 0:
        ...
        if MAX_PROFIT_AMOUNT <= 0:
            ...
            sys.exit(0)
        MAX_DAILY_PROFIT_AMOUNT = -1
        print("单日利润上限: 已禁用（Challenge 阶段不设日盈 cap）")
    else:
        MAX_PROFIT_AMOUNT = -1
        print("账户止盈: 已禁用（Funded 无利润目标）")
        while True:
            try:
                default_cap = str(int(DEFAULT_FUNDED_DAY_PROFIT_CAP))
                cap_str = input(
                    f"Funded 单日利润上限 USD（一致性 15%；回车={default_cap}；0=禁用）: "
                ).strip() or default_cap
                cap = float(cap_str)
                if cap < 0:
                    print("错误: 请输入 >= 0 的数字")
                    continue
                MAX_DAILY_PROFIT_AMOUNT = cap if cap > 0 else -1
                break
            except ValueError:
                print("错误: 请输入有效数字")
            except EOFError:
                print("错误: 无法读取输入（非交互环境），程序退出")
                sys.exit(1)
        if MAX_DAILY_PROFIT_AMOUNT > 0:
            min_for_consistency = MAX_DAILY_PROFIT_AMOUNT / 0.15
            print(f"单日利润上限: ${MAX_DAILY_PROFIT_AMOUNT:.2f}（达到后平仓且当日不再开仓）")
            print(f"  → 按一致性 15%，累计利润至少约 ${min_for_consistency:.0f} 才便于提款")
        else:
            print("单日利润上限: 已禁用（注意一致性 15% 可能卡出金）")

    print("日内止损: 由 MT5 EA 端执行（真实权益监控）；本脚本镜像 EA halted + Funded 日盈 cap")
    print_phase_ea_checklist(phase)
...
def apply_instance_paths(instance_id):
    """按实例编号隔离日志与信号库；行情缓存仍共用。与普通 aqua_* 隔离。"""
    global INSTANCE_ID, LOG_FILE, DB_PATH
    INSTANCE_ID = int(instance_id)
    LOG_FILE = f"trading_aqua_paylater_{INSTANCE_ID}.log"
    DB_PATH = os.path.join(get_common_files_dir(), f"trading_signals_aqua_paylater_{INSTANCE_ID}.db")
    return INSTANCE_ID


def prompt_instance_id():
    """只需输入 1/2/3/4，自动隔离 DB/日志；与 EA InstanceId 对齐。"""
    env_id = os.environ.get("AQUA_PAYLATER_INSTANCE_ID", "").strip()
    while True:
        try:
            raw = env_id if env_id else input("Aqua Pay Later 编号 (1/2/3/4): ").strip()
            env_id = ""
            n = int(raw)
            if n < 1:
                print("请输入 >= 1 的整数")
            ...
            print(f"✓ PayLater #{INSTANCE_ID}  →  {os.path.basename(DB_PATH)}  /  {LOG_FILE}")
            print(f"  Magic = {AQUA_PAYLATER_MAGIC_BASE + INSTANCE_ID}")
            print(f"  MT5 EA 只需把 InstanceId 设为 {INSTANCE_ID}（用 SQLiteSignalEA_aqua_paylater）")
            return INSTANCE_ID
        except ValueError:
            print("请输入数字，例如 1")
        except EOFError:
            print("非交互环境请设置 AQUA_PAYLATER_INSTANCE_ID=1")
            sys.exit(1)
...
def check_ea_external_close():
    ...
    Aqua Pay Later：浮亏封号约 -1%；EA 用单笔硬止损(~0.95%)挂在 broker 兜底：触及时 MT5 持仓被打掉，
...
def daily_loss_monitor_thread(symbol, position_data):
    """
    日内止盈止损监控线程
    Challenge: 账户 +3% 止盈；Funded: 单日利润 cap（一致性）；日亏由 EA 权威执行
    """
    global DAILY_STOP_TRIGGERED, FORCE_CLOSE_POSITION, DAILY_LOSS_MONITOR_ACTIVE
    global DAILY_PNL, PROFIT_TARGET_TRIGGERED, DAILY_PROFIT_CAP_TRIGGERED
    global EA_MIRROR_CLOSE, EA_MIRROR_CLOSE_REASON
    
    print(f"[{get_us_eastern_time().strftime('%Y-%m-%d %H:%M:%S')}] === 日内止盈止损监控线程已启动 ===")
    print(f"[{get_us_eastern_time().strftime('%Y-%m-%d %H:%M:%S')}] 阶段: {ACCOUNT_PHASE}")
    if MAX_PROFIT_AMOUNT > 0:
        print(f"[{get_us_eastern_time().strftime('%Y-%m-%d %H:%M:%S')}] 账户止盈目标: ${MAX_PROFIT_AMOUNT:.2f}")
    else:
        print(f"[{get_us_eastern_time().strftime('%Y-%m-%d %H:%M:%S')}] 账户止盈: 已禁用")
    if MAX_DAILY_PROFIT_AMOUNT > 0:
        print(f"[{get_us_eastern_time().strftime('%Y-%m-%d %H:%M:%S')}] 单日利润上限: ${MAX_DAILY_PROFIT_AMOUNT:.2f}")
    else:
        print(f"[{get_us_eastern_time().strftime('%Y-%m-%d %H:%M:%S')}] 单日利润上限: 已禁用")
    ...
    while DAILY_LOSS_MONITOR_ACTIVE:
        try:
            ...
            # 使用锁保护共享变量
            with pnl_lock:
                # 如果已经触发止损/止盈/日盈 cap，停止监控
                if DAILY_STOP_TRIGGERED or PROFIT_TARGET_TRIGGERED or DAILY_PROFIT_CAP_TRIGGERED:
                    break
            ...
            if is_trading_hours:
                ...
                # Funded: 单日利润上限（一致性 15%）
                if MAX_DAILY_PROFIT_AMOUNT > 0 and current_daily_pnl >= MAX_DAILY_PROFIT_AMOUNT:
                    print(f"\n[{now.strftime('%Y-%m-%d %H:%M:%S')}] !!!!! [监控线程] 达到 Funded 单日利润上限 !!!!!")
                    print(f"[{now.strftime('%Y-%m-%d %H:%M:%S')}] [监控线程] 当日盈利: ${current_daily_pnl:.2f}")
                    print(f"[{now.strftime('%Y-%m-%d %H:%M:%S')}] [监控线程] 日盈上限: ${MAX_DAILY_PROFIT_AMOUNT:.2f}（一致性 15%）")
                    
                    with pnl_lock:
                        FORCE_CLOSE_POSITION = True
                        DAILY_PROFIT_CAP_TRIGGERED = True
                    
                    print(f"[{now.strftime('%Y-%m-%d %H:%M:%S')}] [监控线程] 已设置平仓标志，今日不再开仓")
                    break
                ...
                # Pay Later：硬止损/浮亏封号打掉实盘仓 → 通知主循环镜像平仓（不 halt 当日）
...
def run_trading_strategy(symbol=SYMBOL, check_interval_minutes=CHECK_INTERVAL_MINUTES,
    ...
    global DAILY_LOSS_MONITOR_ACTIVE, FORCE_CLOSE_POSITION, DAILY_PROFIT_CAP_TRIGGERED
    global EA_MIRROR_CLOSE, EA_MIRROR_CLOSE_REASON
    ...
    while True:
        ...
        if FORCE_CLOSE_POSITION and position_quantity != 0:
            ...
            if entry_price and current_price > 0:
                ...
                with pnl_lock:
                    DAILY_PNL += pnl
                    TOTAL_PNL += pnl
                # 记录平仓交易（账户止盈 / 日盈 cap / 日亏）
                if PROFIT_TARGET_TRIGGERED:
                    action_type = "平仓(账户止盈)"
                elif DAILY_PROFIT_CAP_TRIGGERED:
                    action_type = "平仓(日盈上限)"
                else:
                    action_type = "平仓(止损)"
                DAILY_TRADES.append({
        ...
        if last_date is not None and current_date != last_date:
            ...
            DAILY_PROFIT_CAP_TRIGGERED = False  # 重置 Funded 日盈 cap
        ...
        # 已触发止盈/止损/日盈cap或已收盘时不再重启监控线程，避免每分钟重复打印启动/停止日志
        monitor_needed = is_after_930 and current_hour < 16 and not (
            PROFIT_TARGET_TRIGGERED or DAILY_STOP_TRIGGERED or DAILY_PROFIT_CAP_TRIGGERED
        )
        ...
        if position_quantity != 0:
            ...
            if DAILY_PROFIT_CAP_TRIGGERED:
                if LOG_VERBOSE:
                    print(f"[{now.strftime('%Y-%m-%d %H:%M:%S')}] 已触发单日利润上限，跳过开仓检查")
                continue
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("长桥API交易策略启动 - 模拟模式 (AquaFunded Pay Later One-Step)")
    print("=" * 60)

    print("\n--- 实例隔离（多账户必填；与普通 aqua 分开）---")
    prompt_instance_id()

    # 启用日志记录（须在确定 INSTANCE_ID / LOG_FILE 之后）
    sys.stdout = Logger(LOG_FILE)
    sys.stderr = sys.stdout  # 错误信息也记录到日志
    print(f"[PayLater 实例 {INSTANCE_ID}] 日志已写入 {os.path.abspath(LOG_FILE)}")
    
    print("\n--- 账户资金 / Challenge·Funded 阶段设置 ---")
    prompt_capital_settings()
    
    print("版本: 1.0.0-paylater")
    print("时间:", get_us_eastern_time().strftime("%Y-%m-%d %H:%M:%S"), "(美东时间)")
    print(f"Aqua Pay Later 实例编号: {INSTANCE_ID}")
    print(f"阶段: {ACCOUNT_PHASE}")
    ...
    print(f"账户止盈目标: ${MAX_PROFIT_AMOUNT:.2f} ({'已禁用' if MAX_PROFIT_AMOUNT <= 0 else '已启用'})")
    print(f"单日利润上限: ${MAX_DAILY_PROFIT_AMOUNT:.2f} ({'已禁用' if MAX_DAILY_PROFIT_AMOUNT <= 0 else '已启用'})")
    print("日内止损: 由 MT5 EA 端执行（真实权益逐tick监控 + broker服务器端SL）")
    print("⚠️ Funded 出金/重置后：请在 MT5 EA 勾选 ForceResetDailyRisk 加载一次以重算日内锚点")
    print(f"交易时间: {TRADING_START_TIME[0]:02d}:{TRADING_START_TIME[1]:02d} - {TRADING_END_TIME[0]:02d}:{TRADING_END_TIME[1]:02d}")
