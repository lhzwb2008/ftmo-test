...
def prompt_capital_settings():
    ...
    while True:
        try:
            ...
            current_str = input("请输入账户当前金额（如 204000）: ").strip()
    ...
    # Funded 账户：默认 1.5x；可选 0.5x（前 14 天保底 refund）
    if phase == "funded":
        while True:
            try:
                lev_str = input(
                    f"Funded 杠杆选择（回车=默认 {PHASE_LEVERAGE['funded']}x / 输入 0.5=保底refund）: "
                ).strip()
                if lev_str == "":
                ...
                if lev in (0.5, 1.5):
                    LEVERAGE = lev
                    break
                print("错误: Funded 杠杆仅支持 0.5 或 1.5，请重新输入")
            except ValueError:
                print("错误: 请输入 0.5，或直接回车使用默认 1.5x")
            except EOFError:
                print("错误: 无法读取输入（非交互环境），程序退出")
                sys.exit(1)
    ...
    if phase == "funded":
        mode_note = "保底refund / 最低风险" if LEVERAGE == 0.5 else "默认"
...
DB_PATH = os.path.join(get_common_files_dir(), "trading_signals_ftmo.db")
